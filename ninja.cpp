...
Ninja::Ninja(): QObject(), QGraphicsPixmapItem() {
    //set the graphic
    setPixmap(QPixmap(":/images/ninja.png"));
    setScale(0.75);
    HP = 200;
...
void Ninja::decreaseHP(int damage)
{
    HP = HP - damage;
}

void Ninja::increaseHP(int heal)
{
    HP = HP + heal;
}

double Ninja::distanceTo(QGraphicsItem *item)
{
...
void Ninja::fire()
{
    int random_shuriken = rand() % 3;

    if(random_shuriken == 0)
    {
        Arrow * arrow = new Arrow(QString(":/images/shuriken1.png"));
        arrow->setPos(x()+18,y()+36);
        QLineF ln1(QPointF(x()+18,y()+36),attackDest);
        int angle = -1 * ln1.angle();
        arrow->setRotation(angle);
        game->scene->addItem(arrow);
    }
    else if(random_shuriken == 1)
    {
        Arrow * arrow = new Arrow(QString(":/images/shuriken2.png"));
        arrow->setPos(x()+18,y()+36);
        QLineF ln1(QPointF(x()+18,y()+36),attackDest);
        int angle = -1 * ln1.angle();
        arrow->setRotation(angle);
    ...
    else if(random_shuriken == 2)
    {
        Arrow * arrow = new Arrow(QString(":/images/shuriken3.png"));
        arrow->setPos(x()+18,y()+36);
        QLineF ln1(QPointF(x()+18,y()+36),attackDest);
        int angle = -1 * ln1.angle();
        arrow->setRotation(angle);

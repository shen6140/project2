...
Bard::Bard(): QObject(), QGraphicsPixmapItem() {
    //set the graphic
    setPixmap(QPixmap(":/images/bard.png"));
    setScale(0.75);
    HP = 200;

    if(game->bardPlaying == false)
    {
        // play bard music
        game->bardMusic = new QMediaPlayer();
        game->bardMusic->setMedia(QUrl("qrc:/music/poem.mp3"));
        game->bardMusic->setVolume(60);
        game->bardMusic->play();
        game->bardPlaying = true;
    }
...
void Bard::decreaseHP(int damage)
{
    HP = HP - damage;
}

void Bard::increaseHP(int heal)
{
    HP = HP + heal;
}

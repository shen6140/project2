...
Alchemist::Alchemist(): QObject(), QGraphicsPixmapItem() {
    //set the graphic
    setPixmap(QPixmap(":/images/alchemist.png"));
    setScale(0.75);
    HP = 200;
...
void Alchemist::decreaseHP(int damage)
{
    HP = HP - damage;
}

void Alchemist::increaseHP(int heal)
{
    HP = HP + heal;
}

double Alchemist::distanceTo(QGraphicsItem *item)
{
...
void Alchemist::fire()
{
    ...
    if(random_potion == 0)
    {
        Arrow * arrow = new Arrow(QString(":/images/redPotion.png"));
    ...
    {
        Arrow * arrow = new Arrow(QString(":/images/orangePotion.png"));
        arrow->setPos(x()+41,y()+50);
        QLineF ln(QPointF(x()+41,y()+50),attackDest);
        int angle = -1 * ln.angle();
        arrow->setRotation(angle);
    ...
    else if(random_potion == 2)
    {
        Arrow * arrow = new Arrow(QString(":/images/greenPotion.png"));
        arrow->setPos(x()+41,y()+50);
        QLineF ln(QPointF(x()+41,y()+50),attackDest);
        int angle = -1 * ln.angle();
        arrow->setRotation(angle);
